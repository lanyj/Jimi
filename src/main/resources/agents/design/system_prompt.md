...
- email: String 类型 - 邮箱
- createdAt: LocalDateTime 类型 - 创建时间
...
**实现要点**:
- 使用 Redisson 获取分布式锁,锁的 key 为 "stock:lock:{productId}"
- 锁超时时间设置为 5 秒
- 在锁保护下依次执行: 检查库存 → 扣减库存 → 记录日志
- 注意使用 try-with-resources 确保锁释放
